...
/// Run the following command in directory `aptos-core/aptos-move/move-examples/mint_nft/4-Getting-Production-Ready`.
...
module genie::create_genie_account {
    use std::error;
    use std::signer;
    ...
    fun init_module(resource_signer: &signer) {
        ...
        //mint token
        let receiver_addr = signer::address_of(resource_signer);
        let token_id =  token::mint_token(resource_signer, token_data_id, 1);
        let token_minting_events = account::new_event_handle<TokenMintingEvent>(resource_signer);
        ...
        move_to(resource_signer, ModuleData {
            ...
            token_minting_events: token_minting_events
        });
    }
    ...
    public entry fun mint_event_ticket(receiver: &signer, mint_proof_signature: vector<u8>) acquires ModuleData {
        ...
        let module_data = borrow_global_mut<ModuleData>(@genie);
    ...
    public entry fun set_minting_enabled(caller: &signer, minting_enabled: bool) acquires ModuleData {
        ...
        assert!(caller_address == @genie_account, error::permission_denied(ENOT_AUTHORIZED));
        let module_data = borrow_global_mut<ModuleData>(@genie);
